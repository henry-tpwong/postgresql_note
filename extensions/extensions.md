...
### I. PostgreSQL 原生分區能力摘要

> PG 16 已將宣告式分區打磨成熟：RANGE / LIST / HASH 三種策略、partition pruning 執行階段跳過無關分區、ATTACH/DETACH CONCURRENTLY 非阻塞維護、邏輯複製完整支援。大部分時間維度分區場景無需第三方工具即可運作，pg_partman 的價值轉向**自動化生命週期管理**（預建未來分區、定時清理過期資料）。

#### a. 現代分區核心能力表

| 能力 | 支援版本 | 白話解釋 |
|------|---------|---------|
| RANGE / LIST 分區 | PG 10+ | 按範圍或列舉值分割資料 |
| HASH 分區 | PG 11+ | 按 hash 值均勻分布到 N 個分區 |
| DEFAULT 分區 | PG 11+ | 捕獲不匹配任何規則的行，防止 INSERT 失敗 |
| ATTACH / DETACH PARTITION | PG 10+ | 線上掛載/解除分區，無需重建父表 |
| ATTACH CONCURRENTLY | PG 15+ | 非阻塞 ATTACH，不阻塞查詢寫入 |
| Partition Pruning | PG 10+（PG 12 大幅重構） | Planner/Executor 自動跳過不相關分區 |
| 跨分區 UPDATE 自動遷移 | PG 11+ | UPDATE 改分區鍵值時自動 DELETE + INSERT |
| Foreign Key 指向分區表 | PG 12+ | 子表 FK 可參照分區父表 |
| 邏輯複製分區表 | PG 13+ | 單一 Publication 涵蓋所有子分區 |
| MERGE on 分區表 | PG 16+ | UPSERT 語法直接作用於分區父表 |

#### b. PG 16-17 最新增強

PG 16 重點在 JOIN 查詢的分區裁剪改進——更多 JOIN 類型與 Incremental Sort 節點可參與 partition pruning，讓複雜 OLAP 查詢也能有效跳過無關分區。PG 17 補上最後一塊拼圖：`MERGE`（UPSERT）可直接對分區表執行，內部自動路由到正確子分區，無需應用層先查詢分區鍵再決定目標表。

```mermaid
timeline
    title PostgreSQL 宣告式分區里程碑
    PG 10 : Declarative Partitioning : RANGE / LIST : ATTACH / DETACH
    PG 11 : HASH 分區 : DEFAULT 分區 : UPDATE 跨分區遷移
    PG 12 : Partition Pruning 重構 : FK 參照分區表
    PG 13 : 分區表邏輯複製
    PG 16 : JOIN 分區裁剪擴展 : Incremental Sort 裁剪
    PG 17 : MERGE on partitioned tables
```

```sql
-- PG 15+ ATTACH CONCURRENTLY：非阻塞掛載新分區
CREATE TABLE orders_2026_06 (LIKE orders INCLUDING DEFAULTS INCLUDING CONSTRAINTS);
ALTER TABLE orders ATTACH PARTITION orders_2026_06
    FOR VALUES FROM ('2026-06-01') TO ('2026-07-01');

-- PG 16 Partition Pruning 驗證：JOIN + 時間範圍過濾
EXPLAIN (ANALYZE, SUMMARY OFF)
SELECT o.id, oi.product_id, oi.quantity
FROM orders o
JOIN order_items oi ON o.id = oi.order_id
WHERE o.created_at >= '2026-04-01' AND o.created_at < '2026-04-08';

-- 預期輸出：Subplans Removed: N（不相關月份分區被跳過）
```
### II. pg_partman 核心概念
...
### V. 完整示例：訂單表按天分區（PG 16）

#### a. ：建立分區父表並配置 pg_partman
...
#### d. ：EXPLAIN 展示 Partition Pruning
...
#### d. 方式二：使用 userlist.txt 驗證（不依賴 PG auth）
...
#### b. PgBouncer 1.22+ Prepared Statement 追蹤
...
#### c. _in_transaction_session_timeout 協調
...
#### e. Leak Detection（透過 SHOW CLIENTS）
...
### V. VACUUM FULL vs pg_repack vs pg_squeeze 對比
...
## 生產環境 pg_repack 包裝腳本 — 配合 crontab / pg_cron 排程執行
set -euo pipefail
...
TABLE_SAFE="${TABLE_NAME//./_}"
LOG_FILE="$LOG_DIR/repack_${TABLE_SAFE}_${TIMESTAMP}.log"
mkdir -p "$LOG_DIR"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*" | tee -a "$LOG_FILE"; }

log "=== pg_repack 開始：$TABLE_NAME ==="

## 1. Disk check — 確保可用空間 ≥ 表大小 × 2
PGDATA=$(psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -Atc "SHOW data_directory")
DISK_AVAIL_GB=$(df -BG "$PGDATA" | awk 'NR==2 {print $4}' | sed 's/G//')
TABLE_SIZE_GB=$(psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -Atc \
    "SELECT pg_total_relation_size('$TABLE_NAME') / 1073741824")

if [ "$DISK_AVAIL_GB" -lt $((TABLE_SIZE_GB * 2)) ]; then
    log "ABORT: disk ${DISK_AVAIL_GB}GB < ${TABLE_SIZE_GB}GB × 2"
    exit 1
fi
log "Disk OK: avail ${DISK_AVAIL_GB}GB, need $((TABLE_SIZE_GB * 2))GB"

## 2. Replication lag check — 避免 repack 期間 WAL 堆積壓垮 replica
REPLICA_LAG=$(psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -Atc \
    "SELECT COALESCE(max(replay_lag)::text, 'no replica') FROM pg_stat_replication")
log "Replica lag: $REPLICA_LAG"

## 3. 記錄 repack 前大小
SIZE_BEFORE=$(psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -Atc \
...
if pg_repack \
    ...
    --wait-timeout="$WAIT_TIMEOUT" --no-superuser-check \
    >> "$LOG_FILE" 2>&1; then
    DURATION=$(( $(date +%s) - START_EPOCH ))
    SIZE_AFTER=$(psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -Atc \
        "SELECT pg_size_pretty(pg_total_relation_size('$TABLE_NAME'))")
    log "DONE: ${DURATION}s, $SIZE_BEFORE → $SIZE_AFTER"
else
...
```bash
## 每月 1 日凌晨 3:00 對 public.events 執行 repack
0 3 1 * * /usr/local/bin/auto_repack.sh mydb public.events 60
```
...
| 步驟 | 目的 |
|------|------|
| 磁碟檢查 (df) | pg_repack 需 ~2x 原表空間容納新舊兩表，避免執行到一半磁碟滿 |
| Replica lag 檢查 | repack 產生大量 WAL，若 streaming replica 已有延遲會雪上加霜 |
| 大小記錄 | repack 前後對比 size，確認空間有實際回收 |
| 結果寫入日誌 | 所有輸出進 `$LOG_DIR`，事後可追溯每次 repack 的耗時與成效 |
...
#### d. Transaction ID Wraparound 考量
...
| `jobname` | `text` | 任務名稱（PG 16+），用於 `human-friendly` 識別，可選 |

#### c. cron.job_run_details 執行日誌
...
SELECT cron.unschedule('vacuum-off-peak');  -- 依 jobname 取消（PG 16+）
```
...
#### d. 數據清理（GDPR Retention、Log Archival）
...
#### f. cron.max_job_execution_time（PG 16）Timeout 保護
...
#### b. 實戰查詢 — 與 pg_stat_statements JOIN
...
#### g. hypopg_list_indexes() — 列出所有假設索引
...
### I. 核心價值 — 與 `pg_stat_statements` 互補
...
#### b. 與 `log_statement` / `log_duration` 的分工
...
#### d. PG 16 `log_nested_statements`：預存程序偵錯利器
...
| 函數 | 語法 | 說明 | 適用場景 |
|------|------|------|----------|
| `similarity` | `similarity(s1, s2)` | 兩個字串的 trigram 重疊比例，範圍 0.0 ~ 1.0 | 短字串模糊匹配（暱稱、地名、SKU） |
| `show_trgm` | `show_trgm(s)` | 回傳文字被拆解後的 trigram 陣列（`text[]`） | 除錯、理解匹配行為 |
| `show_limit` | `show_limit()` | 回傳當前 session 的 `pg_trgm.similarity_threshold` | 確認當前相似度門檻設定 |
| `set_limit` | `set_limit(r)` | 設定 `similarity_threshold`，回傳舊值 | 動態調整查詢靈敏度 |
| `word_similarity` | `word_similarity(haystack, needle)` | 在 haystack 中找最像 needle 的單詞片段（不對稱） | 多詞文本中的關鍵詞匹配 |
| `strict_word_similarity` | `strict_word_similarity(haystack, needle)` | 同上，但要求精確詞邊界對齊 | 嚴格關鍵詞比對（英文效果好） |
| `%` | `s1 % s2` | 等價於 `similarity(s1, s2) > similarity_threshold` | 最常用的模糊查詢運算符 |
| `<%` | `haystack <% needle` | 等價於 `word_similarity(haystack, needle) > word_similarity_threshold` | 單詞級模糊搜尋 |
| `<<%` | `haystack <<% needle` | 等價於 `strict_word_similarity(haystack, needle) > strict_word_similarity_threshold` | 嚴格單詞邊界搜尋 |
...
### VII. PG 16 環境下的 pg_trgm

PG 16 環境下，`pg_trgm` 已受益於多版本累積優化：CJK 多位元組 GIN 加速（PG 14）、並行 GIN 掃描（PG 15）、ICU collation 整合（PG 16-17）。所有功能在 PG 16 均可直接使用，無需額外配置。

```mermaid
flowchart LR
    subgraph PG16_ECOSYSTEM["🐘 PG 16 pg_trgm 生態"]
        direction TB
        CORE["📦 pg_trgm Core<br/>similarity / word_similarity<br/>strict_word_similarity"]
        GIN["🚀 GIN 索引加速<br/>PG 14: multi-byte CJK 優化<br/>PG 15: parallel index scan"]
        ICU["🌐 ICU Collation<br/>PG 16-17: ILIKE 非 ASCII 行為一致"]
        OPS["🔧 運算符<br/>% / <% / <<% / LIKE / ILIKE / ~"]
    end

    APP["🖥️ .NET Application<br/>Dapper / Npgsql"] --> CORE
    APP --> OPS
    CORE --> GIN
    OPS --> GIN
    OPS --> ICU

    style PG16_ECOSYSTEM fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    style CORE fill:#e3f2fd,stroke:#1565c0
    style GIN fill:#fff3e0,stroke:#e65100
    style ICU fill:#f3e5f5,stroke:#7b1fa2
...
`usagecount` 是 `pg_buffercache` 中最關鍵的效能指標，值域 `0` ~ `5`，由 PostgreSQL 的 **Clock-Sweep（時鐘掃描）** 緩存淘汰演算法決定。PostgreSQL 在 shared_buffers 內維護一個環形陣列與一支「時鐘指針」：backend 每次存取某個 buffer 中的 page 時，將該 buffer 的 `usagecount` +1（上限 `5`）；當需要騰出空間載入新 page 時，指針依序掃過 buffer 並將 `usagecount` -1，直到找到 `usagecount = 0` 且未被 pin 的 buffer 作為淘汰候選。
...
stateDiagram-v2
    ...
    Empty --> UC0: 載入新 page（Ring Buffer / Seq Scan）
...
|------------|---------|---------|
| `5` | 極度熱門（B-tree root page、高並發 PK lookup） | 幾乎不會被淘汰 |
...
### IV. 前瞻：PG 17 pg_buffercache_summary()

> **補充（Senior Dev）**
> PG 17 引入了 `pg_buffercache_summary()`，直接讀取 shared memory 中的全域計數器，以 O(1) 常數時間返回 shared_buffers 的單行聚合摘要（`buffers_total`、`buffers_used`、`buffers_dirty`、`buffers_pinned`），無需遍歷 buffer pool descriptor array。對比 PG 16 `SELECT count(*) FROM pg_buffercache` 在 64GB shared_buffers（~838 萬個 buffers）場景需 30~80ms，`pg_buffercache_summary()` 僅需 < 1ms。若你仍在使用 PG 16，建議降低 `pg_buffercache` 的查詢頻率（每 30 分鐘一次），並對結果做快取。

```sql
-- PG 17 專用：一行查詢完成緩存健康檢查
