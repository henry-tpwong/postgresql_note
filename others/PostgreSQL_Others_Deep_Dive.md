...
**什麼是 WAL 堆積？** WAL（Write-Ahead Log，預寫日誌）是 PostgreSQL 保證資料不丟失的核心機制。當啟用歸檔（archive_mode=on）或複製槽（replication slot）時，已提交的 WAL 檔案不能被自動清理，必須等歸檔成功或備機讀取完畢。如果歸檔失敗或備機延遲，WAL 檔案會不斷堆積直到磁碟爆滿，導致資料庫無法繼續寫入。

```mermaid
sequenceDiagram
    participant DBA as DBA/開發者
    participant PG as PostgreSQL
    participant LOCK as 表鎖定機制
    participant APP as 應用程式

    DBA->>PG: BEGIN;
    DBA->>PG: SET LOCAL lock_timeout = '10s';
    DBA->>PG: ALTER TABLE t ADD COLUMN ...;
    PG->>LOCK: 嘗試獲取 AccessExclusiveLock
    
    alt 10 秒內拿到鎖
        LOCK-->>PG: 鎖定成功
        PG-->>DBA: ALTER TABLE 完成
    else 超時
        LOCK-->>PG: 鎖定失敗 (timeout)
        PG-->>DBA: ERROR: canceling statement due to lock timeout
    end
    
    Note over PG,APP: lock_timeout 期間 APP 的查詢被阻塞
    DBA->>PG: COMMIT;
    PG-->>APP: 查詢恢復正常
```
...
PostgreSQL 每年發布一個大版本，每個版本都有大量改進。有些過去因為 PostgreSQL 限制而制定的規則，在新版本中可能已經不再適用。本章標記了 4 條已過時的規則，並列出 PG 14-17 新增的相關功能。

**為什麼 hash index 曾經被禁用？** 在 PostgreSQL 9.6 及更早版本中，hash index 的變更不會被記錄到 WAL（預寫日誌）中。這意味著如果資料庫突然崩潰，重啟後 hash index 中的資料可能不完整或損壞。從 PostgreSQL 10 開始，hash index 的變更會正確寫入 WAL，崩潰恢復時可以重建，因此現在可以安全使用。

**為什麼過去說分區數不宜過多？** PostgreSQL 9.6 以前的分區實作是基於繼承（table inheritance），優化器在規劃查詢時需要評估所有分區，分區越多規劃時間越長。PostgreSQL 10 引入了宣告式分區（declarative partitioning），加上 11-17 版本持續優化的分區修剪（partition pruning）機制，現在即使有上千個分區也不會明顯影響效能。

```mermaid
timeline
    title PostgreSQL 版本演進與規範影響
    PG 9.4 : GIN lock 問題 (已過時)
    PG 9.5 : SKIP LOCKED<br>array_position
    PG 9.6 : Parallel Query<br>Hash index 仍不寫 WAL
    ...
    PG 14 : CYCLE/SEARCH CTE<br>GIN pending list 優化
    PG 15 : MERGE statement<br>SQL/JSON path
    PG 16 : pg_stat_io<br>Parallel VACUUM FULL<br>geqo_threshold 調整
...
**為什麼要用 Event Trigger 而不是普通 Trigger？** 普通 Trigger 掛在具體的表上，只能偵測該表的 INSERT/UPDATE/DELETE。DDL 操作（如 ALTER TABLE）不會觸發普通 Trigger。Event Trigger 是 PostgreSQL 9.3+ 引入的特殊機制，可以在 DDL 命令執行前後自動觸發——不限於特定表，而是整個資料庫層級。
...
sequenceDiagram
    ...
    Note over PG: sql_drop event 可攔截 (PG 9.5+)
...
PostgreSQL 每個大版本都會對現有功能進行改進。pg_trgm 從 PG 9.1 引入至今已相當成熟，但每個版本仍有細節優化。

**PG 15 的 ICU collation 支援是什麼？** 傳統 PostgreSQL 依賴作業系統的 locale 設定（如 `en_US.UTF-8`），這在不同作業系統間可能不一致。ICU（International Components for Unicode）是跨平台統一的國際化庫。PG 15 讓 pg_trgm 可以配合 ICU collation 正確處理多語言文字的分詞和 trigram 提取。

**PG 17 的 GIN Parallel Build 是什麼？** 傳統上建 GIN 索引是單執行緒操作。PG 17 讓 GIN 索引建立可以利用多核心平行處理——對於數 TB 級的大表，建索引時間可以大幅縮短。

```mermaid
timeline
    title pg_trgm 功能演進
    PG 9.1 : pg_trgm 首次引入<br>LIKE/ILIKE 加速
    PG 9.3 : regex ~ 支援 GIN 加速
    PG 9.6 : pg_trgm 支援相似度搜尋
    PG 14 : GIN pending list 清理優化<br>word_similarity_threshold 更靈活
    PG 15 : ICU collation 支援<br>regex engine 效能提升
    PG 16 : strict_word_similarity<br>multi-byte encoding 優化
    PG 17 : GIN parallel index build 支援<br>大規模建索引更快
```
...
**現代替代方案**：PostgreSQL 9.5+ 內建了 `array_position()` 函數（相當於 C 語言效能），PG 14+ 的 `get_bit()` / `set_bit()` 對 varbit 的操作也已足夠高效，大部分自訂函數在較新版本的 PostgreSQL 中已有內建替代。

```mermaid
flowchart LR
    subgraph "阿里雲 RDS PG 客製化"
        C1["bit_count_range_zero()"] --> U1["統計區段剩餘票數"]
        C2["array_pos() C 版本"] --> U2["O(1) 站點位置查詢"]
        C3["varbit 批量操作"] --> U3["一次更新多位元"]
    end
    
    subgraph "PG 原生已支援 (較新版本)"
        N1["PG 9.5+: array_position()"]
